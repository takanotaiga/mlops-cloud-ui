export const DB_OPERATION_SQL = {
  ...
  inferenceJobCreate: "CREATE inference_job CONTENT { name: $name, dead: false, status: 'ProcessWaiting', taskType: $taskType, model: $model, modelSource: $modelSource, inferenceBackend: $inferenceBackend, rtdetrEpochs: $rtdetrEpochs, datasets: $datasets, createdAt: time::now(), updatedAt: time::now() }",
  inferenceJobUpdate: "UPDATE inference_job SET dead = false, status = 'ProcessWaiting', taskType = $taskType, model = $model, modelSource = $modelSource, inferenceBackend = $inferenceBackend, rtdetrEpochs = $rtdetrEpochs, datasets = $datasets, updatedAt = time::now() WHERE name == $name",
